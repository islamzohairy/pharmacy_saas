...
/// profile (so the providers resolve a pharmacyId) and the customer debt
/// screen as the initial route.
Future<ProviderContainer> pumpCustomersApp(
  WidgetTester tester,
  AppDatabase db, {
  ...
  await tester.pumpWidget(
    UncontrolledProviderScope(
      container: container,
      child: PharmacyApp(
        router: buildRouter(initialLocation: AppRoutes.customerDebt),
      ),
    ),
  );
...
void main() {
  ...
  group('CustomerDebtScreen', () {
    testWidgets('shows the empty state with an add FAB', (tester) async {
      await pumpCustomersApp(tester, db, profileId: profileId);

      expect(find.text('لا يوجد عملاء بعد'), findsOneWidget);
      expect(find.byType(FloatingActionButton), findsOneWidget);
    });

    testWidgets('creates a customer and shows a zero balance', (tester) async {
      await pumpCustomersApp(tester, db, profileId: profileId);

      await tester.tap(find.byType(FloatingActionButton));
      await tester.pumpAndSettle();
      await tester.enterText(
        find.widgetWithText(TextField, 'اسم العميل'),
        'أم علي',
      );
    ...
    testWidgets(
      'records a customer debt as one ledger row and updates the balance',
      (tester) async {
        final customerId = await seedCustomer(db, pharmacyId, name: 'أم علي');
        await pumpCustomersApp(tester, db, profileId: profileId);
        ...
        expect(rows.single.type, LedgerEntryType.customerDebt);
        expect(rows.single.customerId, customerId);
    ...
    testWidgets('validates the debt amount in the dialog', (tester) async {
      await seedCustomer(db, pharmacyId, name: 'أم علي');
      await pumpCustomersApp(tester, db, profileId: profileId);
